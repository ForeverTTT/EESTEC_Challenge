...
| 限制 | 说明 | 提升方向 |
|------|------|----------|
| **仅 2 个麦克风** | PSOC Edge E84 AI Kit 板载双 PDM 麦克风，DOA 信息维度有限 | 扩展为 **4 麦克风阵列**（如四边各一），可覆盖 360° 方向，显著改善前后/左右对称方向的区分能力 |
| **前后方向模糊** | East 与 West（或对称方向）在双麦上的 ITD/ILD 模式相近，测试集 East 召回率仅 74.5% | 四麦布局可引入垂直或前后维度的相位差，打破左右对称歧义 |
| **固定麦距与板型** | 当前模型未显式利用麦克风间距、PCB 尺寸等物理参数 | 将 **麦距、声速、采样率** 等作为先验注入特征或网络结构 |

#### 算法与模型提升

1. **嵌入物理约束与先验知识**
   - 在特征层引入基于物理的 TDOA / GCC-PHAT 估计，与 Log-Mel 特征融合，而非纯数据驱动
   - 将 **麦克风坐标、板级几何尺寸、最大 ITD 上界**（由麦距与声速决定）作为网络输入或损失函数约束，缩小假设空间、提升泛化
   - 对 "不可能" 的方向组合（如超出阵列分辨率的预测）施加物理一致性惩罚

2. **数据与训练**
   - 增大 **East 方向** 及边界样本采集量（当前主要混淆来源）
   - 引入 **数据增强**：混响、背景噪声叠加、不同播放距离与角度微调
   - 在真实警笛录音（非仅 Chirp 信标）上微调，提升实际场景鲁棒性

3. **模型与部署**
   - 在 CM55 算力允许下尝试更深网络或 **角度回归**（连续 DOA）替代四方向硬分类
   - 板端 **在线校准**：针对具体设备做一次短校准，补偿麦增益差异与安装误差
   - 结合 **时序平滑 / 卡尔曼滤波**，利用连续帧间的方向一致性减少抖动误判
